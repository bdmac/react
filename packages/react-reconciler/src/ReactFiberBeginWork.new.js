...
function mountSuspenseState(renderLanes: Lanes): SuspenseState {
  return {
    dehydrated: null,
...
function updateSuspenseState(
  ...
  return {
    dehydrated: null,
    // TODO: Ideally, this would only include the lanes that actually have
    // updates in the ancestor path or in the subtree.
    baseLanes: combineLanes(prevSuspenseState.baseLanes, renderLanes),
    retryLane: NoLane,
  };
}
...
function shouldRemainOnFallback(
  ...
  if (current !== null) {
    ...
    if (suspenseState !== null) {
      // Currently showing a fallback. In order to unhide the context, we must
      // be rendering all the lanes that previously suspendeded this boundary.
      // Otherwise, we must remainin on the fallback, regardless of what the
      // Suspense context says.
      if (!isSubsetOfLanes(renderLanes, suspenseState.baseLanes)) {
        return true;
      }
...
function getRemainingWorkInPrimaryTree(
  ...
  if (currentSuspenseState !== null) {
    // This boundary already timed out. If we previously suspended at other
    // lanes, we can't unhide this fallback without rendering all the base
    // lanes. Those lanes might have been unblocked by a new update, so we
    // should try rendering all of them again.
    const baseLanes = currentSuspenseState.baseLanes;
    if (!isSubsetOfLanes(renderLanes, baseLanes)) {
      return baseLanes;
    }
  }
...
function updateSuspenseComponent(current, workInProgress, renderLanes) {
  ...
  if (current === null) {
    ...
    if (nextDidTimeout) {
      ...
      // fallback children.
      workInProgress.memoizedState = mountSuspenseState(renderLanes);
...
function mountDehydratedSuspenseComponent(
  ...
  if ((workInProgress.mode & BlockingMode) === NoMode) {
    ...
    // }
    workInProgress.lanes = laneToLanes(DefaultBumpedLane);
  } else {
...
function bailoutOnAlreadyFinishedWork(
  ...
  markUnprocessedUpdateLane(workInProgress.lanes);
...
function beginWork(
          ...
          if (state !== null) {
            ...
            if (includesSomeLane(renderLanes, primaryChildLanes)) {
              ...
              while (primaryChild !== null) {
                if (
                  includesSomeLane(renderLanes, primaryChild.lanes) ||
                  includesSomeLane(renderLanes, primaryChild.childLanes)
                ) {
  ...
  workInProgress.lanes = removeLanes(updateLanes);
